...
public class Bookmarking extends DialogFragment implements OnClickListener {

	public Bookmarking(String pid, String realId, boolean mark, int position) {
		this.pid = pid;
		this.uid = realId;
		this.mark = mark;
	...
	public void onSaveInstanceState(Bundle arg0) {
		...
		arg0.putString("pidMarking", pid);
		arg0.putBoolean("markMarking", mark);
		arg0.putInt("posMarking", position);
		arg0.putString("uidMarking", uid);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if (savedInstanceState != null) {
			pid=savedInstanceState.getString("pidMarking");
			uid=savedInstanceState.getString("uidMarking");
			mark=savedInstanceState.getBoolean("markMarking");
			position=savedInstanceState.getInt("posMarking");
		}
	...
	private class ProcessBookmark extends
		...
		protected void onPreExecute() {
			...
			if (mark) {
				tv_messgs.setText("bookmarking...");
			} else {
				tv_messgs.setText("unbookmarking...");
			}
		...
		protected JSONObject doInBackground(String... args) {
			...
			JSONObject json = userFunction.processBookmark(pid, uid);
		...
		protected void onPostExecute(JSONObject json) {
			try {
				if (json.getString(UserFunctions.KEY_SUCCESS) != null) {
					...
					if (Integer.parseInt(res) == 1) {
						...
						if (json.getString("action").equals("a")) {
							Message.message(con, "Added to Bookmark");
						} else {
							Message.message(con, "Removed from Bookmark");
						}
				...
				trigger.refreshFromBookmark(position, pid);
				getDialog().dismiss();
			}
	...
	public void onAttach(Activity activity) {
		...
		trigger = (BookMarkTrig) activity;
	}

	public interface BookMarkTrig {
		public void refreshFromBookmark(int position, String postId);
	}

	BookMarkTrig trigger;
	String pid = "0", uid = "0";
	Context con;
